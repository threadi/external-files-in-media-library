...
class Synchronization extends Tools_Base {
	...
	public function add_column_content_files( string $content, string $column_name, int $term_id ): string {
		...
		// create the dialog for the delete link.
		$dialog = array(
			'className' => 'efml',
			'title'     => __( 'Delete synchronized files?', 'external-files-in-media-library' ),
			'texts'     => array(
				'<p><strong>' . __( 'Are you sure you want to delete these synchronized files?', 'external-files-in-media-library' ) . '</strong></p>',
	...
	public function sync( string $url, array $term_data, int $term_id ): void {
		...
		add_action( 'efmlgd_google_drive_directory_import_files', array( $this, 'set_url_max_count' ), 10, 2 );
	 ...
	 *
	 * @param array<string,mixed> $post_array Given post array for the update.
