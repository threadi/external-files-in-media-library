...
class Files {
	...
	public function add_url( string $url, bool $add_to_queue = false ): bool {
		...
		foreach ( $files as $file_data ) {
			 ...
			 * @param array<string,mixed>  $file_data List of file settings detected by importer.
			 */
			$post_array = apply_filters( 'eml_file_import_attachment', $post_array, $url, $file_data );
			 ...
			 */
			do_action( 'eml_file_import_before_save', $url );
			...
			if ( false !== $file_data['local'] ) {
				// log this event.
				$log->create( __( 'URL will be saved local.', 'external-files-in-media-library' ), $url, 'info', 2 );
				...
				if ( is_string( $file_data['tmp-file'] ) && file_exists( $file_data['tmp-file'] ) ) {
					...
					// log this event.
					$log->create( __( 'Temp-file for import has been deleted.', 'external-files-in-media-library' ), $url, 'info', 2 );
				}
			} else {
				// log this event.
				$log->create( __( 'URL will be stay external, just save the reference in media library.', 'external-files-in-media-library' ), $url, 'info', 2 );
			...
			if ( is_wp_error( $attachment_id ) ) {
				...
				$log->create( sprintf( __( 'URL could not be saved because of the following error: %1$s', 'external-files-in-media-library' ), '<code>' . wp_json_encode( $attachment_id->errors['upload_error'][0] ) . '</code>' ), $url, 'error' );
			...
			if ( ! $external_file_obj ) {
				// log event.
				$log->create( __( 'External file object for URL could not be loaded.', 'external-files-in-media-library' ), $url, 'error' );
			...
			$log->create( __( 'URL successfully added in media library.', 'external-files-in-media-library' ), $url, 'success' );
			$log->create( __( 'Using following settings to save this URL:', 'external-files-in-media-library' ) . ' <code>' . wp_json_encode( $file_data ) . '</code>', $url, 'success', 2 );
