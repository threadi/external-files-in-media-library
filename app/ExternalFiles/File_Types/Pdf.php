...
/**
 * Object to handle PDF files.
 */
class Pdf extends File_Types_Base {
	/**
	...
	public function get_title(): string {
		return __( 'PDFs', 'external-files-in-media-library' );
	}
	...
	public function is_local(): bool {
		return 'local' === get_option( 'eml_pdf_mode' );
	}
	...
	public function is_proxy_enabled(): bool {
		return 'external' === get_option( 'eml_pdf_mode' ) && 1 === absint( get_option( 'eml_pdf_proxy' ) );
	}
	...
	public function is_cache_expired(): bool {
		...
		if ( absint( get_option( 'eml_pdf_proxy_max_age' ) ) <= 0 ) {
			return false;
		}

		// compare cache file date with max proxy age.
		return filemtime( $this->get_file()->get_cache_file() ) < ( time() - absint( get_option( 'eml_pdf_proxy_max_age', 168 ) ) * 60 * 60 );
	}
}
