...
class Import extends Directory_Listing_Base {
	...
	public function add_url( string $url ): bool {
		 ...
		 *
		 * @param int $user_id The title generated by the importer.
		...
		$progress = Helper::is_cli() ? \WP_CLI\Utils\make_progress_bar( _n( 'Save the file from URL', 'Save files from URL', count( $files ), 'external-files-in-media-library' ), count( $files ) ) : '';
		...
		foreach ( $files as $file_data ) {
			 ...
			 * @since 1.1.0 Available since 1.1.0
			 *
			 * @param string $title     The title generated by the importer.
			...
			/**
			 * Run action just before the file is saved in the database.
			 *
			 * @since 2.0.0 Available since 2.0.0.
			 *
			 ...
			 *
			 * Return true if we only import files in the media library without external URL settings.
			 *
			 * @since 5.0.0 Available since 5.0.0.
			 *
			...
			// add the file to local cache, if necessary.
	 ...
	 *
	 * Besides "max_execution_time" other timeouts exist in PHP that we cannot access.
	...
	/**
	 * URL-decode the file-title if it is used in the admin (via AJAX).
	...
	/**
	 * Add a task to set the user agent for any request.
	 *
	 * @return void
	 */
