...
class Forms {
	...
	public function init(): void {
		...
		add_action( 'efmlawss3_s3_directory_import_file_check', array( $this, 'set_import_file_check' ) );
	...
	public function add_styles_and_js_admin( string $hook ): void {
		...
		wp_localize_script(
			...
			array(
				...
				'install_and_activate_service_plugin' => wp_create_nonce( 'efml-install-and-activate-plugin' ),
				'get_install_and_activate_service_plugin_info_nonce' => wp_create_nonce( 'efml-get-install-and-activate-plugin-info-nonce' ),
				...
				'title_service_plugin_progress'       => __( 'Install and activate a service plugin', 'external-files-in-media-library' ),
			)
		);
	}
		...
		if ( 'add' === $current_screen->action ) {
			...
			<div class="eml_add_external_files_wrapper">
				<p>
					...
					echo wp_kses_post( sprintf( __( 'Add external files via their URL <a href="%1$s" target="_blank">here%2$s</a>.', 'external-files-in-media-library' ), esc_url( $url ), Helper::get_a11n_window_hint() ) );
					?>
				</p>
			</div>
			<?php
