...
class Protocol_Base {
	...
	public function get_url_info( string $url ): array {
		_doing_it_wrong( '\ExternalFilesInMediaLibrary\ExternalFiles\Protocol_Base::get_url_info()', __( 'Protocol for a service should use its own get_url_info() handler.', 'external-files-in-media-library' ), '5.4.0' );
		if ( empty( $url ) ) {
			return array();
		}
		return array();
