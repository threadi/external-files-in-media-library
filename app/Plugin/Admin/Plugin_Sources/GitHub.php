...
class GitHub extends Plugin_Sources_Base {
	...
	public function get_description( array $config ): string {
		...
		return '<p>' . sprintf( __( 'The newest release from <a href="%1$s" target="_blank">this GitHub repository%2$s</a> will be loaded.', 'external-files-in-media-library' ), 'https://github.com/' . $config['github_user'] . '/' . $config['github_slug'], Helper::get_a11n_window_hint() ) . '</p>';
	}
}
