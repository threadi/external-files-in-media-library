...
class Log {
	...
	public function get_logs( string $url = '', string $state = '', string $identifier = '' ): array {
		...
		if ( ! empty( $url ) ) {
			if ( ! empty( $state ) ) {
				if ( ! empty( $identifier ) ) {
					return $wpdb->get_results( $wpdb->prepare( 'SELECT `id`, `state`, `time` AS `date`, `log`, `url` FROM ' . $wpdb->prefix . 'eml_logs WHERE 1 = %s AND `url` = %s AND `state` = %s AND `identifier` = %s ORDER BY `time` DESC, `id` DESC LIMIT 1', array( 1, $url, $state, $identifier ) ), ARRAY_A );
				}
				return $wpdb->get_results( $wpdb->prepare( 'SELECT `id`, `state`, `time` AS `date`, `log`, `url` FROM ' . $wpdb->prefix . 'eml_logs WHERE 1 = %s AND `url` = %s AND `state` = %s ORDER BY `time` DESC, `id` DESC LIMIT 1', array( 1, $url, $state ) ), ARRAY_A );
			}
			return $wpdb->get_results( $wpdb->prepare( 'SELECT `id`, `state`, `time` AS `date`, `log`, `url` FROM ' . $wpdb->prefix . 'eml_logs WHERE 1 = %s AND `url` = %s ORDER BY `time` DESC, `id` DESC LIMIT 1', array( 1, $url ) ), ARRAY_A );
		}
