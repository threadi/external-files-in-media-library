<?php
/**
 * File to handle the capability set where only administrators have capabilities.
 *
 ...
 */
class Admin_Dominion extends CapabilitySet_Base {
	 ...
	 * @var string
	 */
	protected string $name = 'admin_dominion';
	...
	public function get_title(): string {
		return __( 'Administrator Dominion', 'external-files-in-media-library' );
	}
	...
	public function run(): void {
		...
		foreach ( Tools::get_instance()->get_tools_as_objects() as $tools_obj ) {
			...
			update_option( 'eml_tools_settings_tools_' . $tools_obj->get_name() . '_allowed_roles', array( 'administrator' ) );
		}
		...
		foreach ( Directory_Listings::get_instance()->get_directory_listings_objects() as $service ) {
			...
			// set the default option.
			update_option( 'eml_service_' . $service->get_name() . '_allowed_roles', array( 'administrator' ) );
		}
	}
}
