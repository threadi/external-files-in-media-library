...
class ZipArchiveBrowser {
	...
	public static function get_contents( Directory_Listing_Base $obj, string $zip_file ): array {
		...
		return self::get_directory_recursively( array(), '/', self::$files );
	}
	 ...
	 * @return array
	 */
	private static function get_directory_recursively( array $list, string $parent_dir, array $directory_list ): array {
		...
		foreach( $directory_list as $item_name => $item_settings ) {
			...
			if( ! isset( $item_settings['name'] ) ) {
				$subs = self::get_directory_recursively( array(), $item_path . '/', $item_settings );
				$entry['dir'] = $item_path;
				$entry['sub'] = $subs;
