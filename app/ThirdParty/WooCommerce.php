...
class WooCommerce {
	...
	public function init_woocommerce(): void {
		...
		$woocommerce_settings_setting->set_field( array(
	 ...
	 * @return void
	 */
	public function add_product_image( WC_Product $object, array $data ): void {
		// bail if no main image is given.
		...
		wc_product_attach_featured_image( $data['eml_file'], $object );
	}
	 ...
	 * @return void
	 */
	public function add_product_gallery_images( WC_Product $object, array $data ): void {
		if( empty( $data['eml_files'] ) ) {
			return;
		}

		$object->set_gallery_image_ids( $data['eml_files'] );
		$object->save();
	}
}
