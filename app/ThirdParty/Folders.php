...
class Folders extends ThirdParty_Base implements ThirdParty {
	...
	public function add_category_selection( string $form, int $term_id ): string {
		...
		// add the HTML-code.
		$form .= '<div><label for="folders_categories">' . __( 'Choose a folder of plugin Folders:', 'external-files-in-media-library' ) . '</label>' . $this->get_folder_selection( $assigned_category ) . '</div>';
