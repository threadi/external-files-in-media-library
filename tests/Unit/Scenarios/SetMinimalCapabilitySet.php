 ...
 */
class SetMinimalCapabilitySet extends externalFilesTests {
	/**
	...
	public function test_set_capability_set(): void {
		...
		$capability_set = \ExternalFilesInMediaLibrary\Plugin\CapabilitySets::get_instance()->get_capability_set_by_name( 'minimal' );
		$this->assertIsObject( $capability_set );
		$this->assertInstanceOf( '\ExternalFilesInMediaLibrary\Plugin\CapabilitySet_Base', $capability_set );
		...
		$this->assertFalse( $test_after );
		$this->assertNotEquals( $test_before, $test_after );
	}
}
